# Harness AI Coding 实战教程

> 本文档对应在线版飞书文档：《Harness AI Coding 实战教程》（https://xiaohuaqb.feishu.cn/docx/MFhcdDA9toZd2sxtGDuc4yRUnuc）。
>
> 这是《Harness Engineering Kit》（仓库内控制系统）的配套实战导览：从"为什么需要工程化"讲到用真实命令把一个空项目接入并完整跑一次非平凡变更。
> 文中图表以**飞书在线版中的白板**呈现（19 块：SVG 架构图 + Mermaid 流程图）；可复用的图源（SVG / Mermaid）在仓库 `_rewrite/diag/` 下。
>
> 建议阅读顺序：先看「为什么需要工程化？」，再读 一 ~ 六 建立概念，第七部分跟着命令跑通一次变更，最后第八部分是全栈并行的进阶。
