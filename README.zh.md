...
`guard --stdin` 是同一门禁的宿主 hook 入口：从 stdin 读取工具调用负载（例如 `{"tool_input":{"file_path":"src/app.ts"}}`），当目标嵌套在别的 unit 内或属于其他 unit 时以退出码 2 拒绝调用；无法识别的负载记为信息项而不是违规，避免陌生的 hook 运行时阻断所有写入。
