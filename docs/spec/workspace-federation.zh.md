...
| unit spec | 某个 unit 自己的 OpenSpec 行为规格变更，位于该 unit 的 `openspec/changes/<change-id>/specs/**/*.md` |
| federation change set | 同一次跨 unit 变更中各 unit 的本地 change/spec 集合；只保存引用关系，不形成新的中央 spec |
...
- 参与同一次跨工程变更的每个 unit 都必须在自身仓内建立独立的 OpenSpec change 和 spec；A、B、C 三个工程都参与时，必须有三份 unit spec，不得合并成 workspace spec。
...
- 不在 workspace 根创建 `openspec/`、`spec.md` 或其他中央 spec；workspace 只保存 federation change set 的引用和投影。
- 不做跨仓自动提交或自动开 PR。
...
| unit spec / change | 各 unit 仓内 `openspec/changes/<change-id>/` | 是 | 是 | 由该 unit 独立提案、审批、实现、归档 |
| federation change set 引用 | 各 unit 的 `governance.json.workspace` | 是 | 是 | 只引用各 unit change，不承载 spec 内容 |
...
unit_kind: frontend                # backend | frontend | mobile | library | docs | infra
repo_url: ssh://git@example.com/coil/backend-ui.git   # 可选；消费者侧兼容校验需要
owner: team-web                    # 可选
...
| `unit_kind` | str | 是 | 取值须在枚举内 |
| `repo_url` | str | 否 | 非空即须为绝对 URL；允许 `https`、`ssh`、`git+ssh`；`publishes` 非空时必填 |
| `owner` | str | 否 | 非占位符 |
| `publishes` | list | 是 | 默认空列表；同一 unit 内 `contract` 不得重复 |
| `consumes` | list | 是 | 默认空列表；同一 unit 内 `contract` 不得重复 |
...
| `consumes[].provider_repo` | str | 是 | 绝对 URL；允许 `https`、`ssh`、`git+ssh` |
| `consumes[].version` | str | 是 | 合法 semver range |
| `consumes[].snapshot` | str | 否 | 归一化 POSIX、仓相对 |
| 顶层键集合 | — | — | 必须恰好为 `{schema_version, kind, workspace_id, unit_id, unit_kind, repo_url, owner, publishes, consumes}` 中去掉可选键后的集合；未知键报错 |
...
```json
{
  ...
  "digest": "sha256:...",
  "units": [
    {"unit_id": "backend-api", "root_index": 0, "path": "backend-api", "harness_root": "backend-api",
...
`units[].root_index` 指向 `roots[]`；`path` 与 `harness_root` 均相对于该 root，使用归一化 POSIX 路径。这样多个 `--root` 时不会因相同仓目录名产生歧义。

`digest` 是所有 unit 的规范化元组
`(unit_id, identity_sha256, kit_version, tracked)` 按 `unit_id` 排序后的内容哈希，用于缓存失效。
仓根路径不进入 digest，因此同一组 unit 更换本地 clone 布局时 digest 保持不变；`.hek/VERSION` 或 Git tracking 状态变化必须使 digest 变化。

`diagnostics[].code` 取值：`workspace.mixed`、`identity.missing`、`identity.schema`、`identity.duplicate`、`identity.untracked`、`unit.nested`、`unit.no-harness`、`contract.orphan`、`contract.provider-mismatch`、`contract.duplicate-provider`、`contract.cycle`、`kit.version-mismatch`、`spec.missing`、`spec.reference`、`spec.duplicate`。
`contract.orphan` 必须带 `level`：provider 未 checkout 时为 `warning`，provider 仓已可见但没有对应 publish 时为 `blocked`。
...
```json
{
  "workspace": {
    ...
    "derived_from": "backend-api-add-export",
    "related_changes": [
      {
        "unit": "backend-api",
        "change_id": "backend-api-add-export",
        "specs": [{"unit": "backend-api", "path": "openspec/changes/backend-api-add-export/specs/export.md"}]
      },
      {
        "unit": "backend-ui",
        "change_id": "backend-ui-update-export",
        "specs": [{"unit": "backend-ui", "path": "openspec/changes/backend-ui-update-export/specs/export.md"}]
      },
      {
        "unit": "client-api",
        "change_id": "client-api-adapt-export",
        "specs": [{"unit": "client-api", "path": "openspec/changes/client-api-adapt-export/specs/export.md"}]
      }
    ],
    "contracts": [
      {
...
约束：`workspace` 段的键集合必须为 `{workspace_id, unit_id, role, change_id, derived_from, related_changes, contracts}` 去掉可选键后的集合；`role ∈ {provider, consumer, independent}`。跨 unit 变更时 `related_changes` 必须包含本 unit 以及每个参与实现的 unit，`unit` 不得重复，且每个成员的 `change_id` 必须等于对应仓 `openspec/changes/<change-id>/` 的目录名；每个成员至少有一项 `specs`，其路径必须存在并位于该成员 unit 的 change 目录内。因此 A、B、C 三个 unit 都参与时，`related_changes` 必须引用三份独立 spec，不允许引用 workspace 根 spec。`specs` 项的键集合必须为 `{unit, path}`，且 `unit` 必须等于所属成员。

本 unit 的 `check_change_workspace.py` 至少校验当前 unit 的 change/spec 文件真实存在，并校验其他成员引用的 `unit`、`change_id`、路径格式；只有聚合 `hek workspace verify` 在所有成员仓均可见时，才校验其他成员 spec 的实际存在和 change 目录归属。外部成员未 checkout 时不得伪造通过，应返回 `spec.missing` blocked。

`contracts` 项的键集合必须为 `{contract, provider, from_version, to_version, breaking, deprecated_until, verification}` 去掉可选键后的集合；每项的 `contract`、`provider`、版本字段必须与 identity/projection 对齐；`breaking` 为布尔值；`breaking` 为 `true` 时必须带 `derived_from`、ISO 日期格式的 `deprecated_until` 和 `verification`；`verification` 必须是 `{unit, path}`，且该 unit 仓内的证据文件必须存在。未发生 breaking 时，`verification` 仍可提供，但提供后同样必须通过路径和文件存在性校验。

### 3.4 多工程 spec 建立规则

这是本方案的强制规则，不是推荐做法：

1. 参与一次 federation change set 的 unit，是本次变更中需要产生实现、配置、测试或契约变更的 unit。每个参与 unit 必须在自身仓内创建一个独立 OpenSpec change，并在该 change 下至少创建一份 `specs/*.md`。
2. 若 A、B、C 三个工程都参与同一次变更，必须存在三份本地 spec，分别位于：

```text
A/openspec/changes/<change-a>/specs/<capability-a>.md
B/openspec/changes/<change-b>/specs/<capability-b>.md
C/openspec/changes/<change-c>/specs/<capability-c>.md
```

3. 三份 spec 的内容边界不同：provider spec 描述 provider 的行为和发布契约；consumer spec 描述本工程如何适配、验证或迁移；第三个工程若同时消费和发布，则同时承担对应的本地行为契约。任何 unit 不得用另一 unit 的 spec 代替自己的实现规格。
4. `related_changes[].specs[]` 只保存引用，不复制 spec 内容。spec 的权威版本始终是其所属 unit 仓内的 OpenSpec 文件；引用失效、change_id 不匹配或 spec 文件缺失均为 blocked。
5. workspace 根不得出现 `openspec/changes/`、`specs/` 或合并后的 `workspace-spec.md`。spec 引用和状态只由各 unit 的 `governance.workspace.related_changes` 提供；workspace projection 可以报告 spec 诊断，但不展示 spec 内容，也不能成为 spec 权威。
6. 同一 unit 可以因多个独立 capability 产生多个本地 spec 文件，但它们仍属于同一个 unit change；“A、B、C 三工程三份 spec”指每个参与 unit 至少一份，不表示禁止单个 unit 的 capability 拆分。
...
| I5 | 每个 `consumes[].contract` 恰好被一个可见 unit `publishes`，且规范化后的 `provider_repo` 与 provider 的 `repo_url` 一致 | 图聚合；区分"未 clone"、"无人发布"和 provider 地址不一致 | blocked / warning |
| I6 | 契约图无环 | DFS | blocked |
| I7 | 跨 unit 契约、证据、派发中的路径必须使用 `{unit, path}`；identity 内的 `artifact`、`snapshot` 等路径天然以所属 unit 为作用域 | 结构校验 + `unit_id` 存在性 | blocked |
...
| I11 | 单任务上下文 `input_bytes` 不超过 `MAX_INPUT_BYTES = 262144`；跨工程任务不得加载无关 unit 的 L1 | `check_context_budget` | blocked |
...
| I14 | 目标路径的 Git 仓根必须等于承载它的 harness 根；不一致即为越界（覆盖嵌套与跨仓）。`verify` 对每个候选 unit 使用其自身 harness 根，`guard` 使用会话根 | `git rev-parse --show-toplevel` + harness 根定位 | blocked |
| I15 | federation change set 中每个参与 unit 都有且仅有一个本地 change 引用，并至少有一份属于该 change 的 spec；不存在 workspace 根 spec | 本 unit 校验本地文件；聚合 verify 校验全部成员文件和路径归属 | blocked |

单仓兼容：仅发现一个 Git 仓且不存在 `identity.yaml` 时，不进入联邦模式，不产生 `identity.missing`，I1–I7、I9–I13 跳过，行为与 0.6.0 完全一致。发现两个或以上 Git 仓时进入联邦模式：所有候选仓都必须有合法且被跟踪的 identity，缺失即 `identity.missing` blocked。**I14 不跳过**——它是 1.0.0 刻意引入的行为变更，对没有 identity 的工程同样生效。
...
3. 记录收集
   对每个候选：先记录仓根（git rev-parse，`.git` 可以是目录或文件）并执行嵌套检测，再读取 `.hek/project/identity.yaml`
   缺失时：单 Git 仓模式不产出诊断；联邦模式记为 `identity.missing` blocked
   记录 unit_id、identity 内容 sha256、`.hek/VERSION`、是否被跟踪
4. 分组
   按 workspace_id 分组；一次 discover/verify 只允许一个 workspace_id，出现多个时返回 `workspace.mixed` 并退出码 2；调用方应拆分 `--root` 集合后分别执行
5. 图构建
   publishes → provider 映射；重复 `unit_id` → `identity.duplicate`；重复 provider → `contract.duplicate-provider`
   consumes → 边；`provider_repo` 不在可见集合 → `contract.orphan`（warning，可能只是未 clone）；provider 仓可见但未发布该 contract → `contract.orphan`（blocked）；地址不一致 → `contract.provider-mismatch`（blocked）
   环检测 → `contract.cycle`
   related_changes → 校验 unit/change/spec 对齐；缺失 → `spec.missing`，引用不一致 → `spec.reference`，重复引用 → `spec.duplicate`
6. 不变量校验（I1–I7、I10、I12、I14、I15；联邦模式下另校验 `identity.missing` 与 `workspace.mixed`）
7. 输出 projection + 缓存
```
...
3. 跨 unit 的契约、证据、派发路径一律 `{"unit": "<unit_id>", "path": "<unit 仓相对路径>"}`；identity 内的 artifact/snapshot 路径以当前 unit 为隐式作用域（I7）。
...
```text
1. 识别本次实际参与的 unit；每个参与 unit 在自己仓内各开一个 OpenSpec change，并各自建立至少一份本地 spec。A/B/C 参与时先形成三份 spec，不建立 workspace 根 spec。
2. 各 unit 在本地完成 proposal、spec、design、tasks 和审批；每份 spec 只描述所属 unit 的行为边界，跨 unit 关系写入 `governance.workspace.related_changes`。
3. provider 修改自己的行为和契约，递增契约版本，打 tag / 发布契约产物（版本可寻址）。
4. 每个 consumer 在自己的 change 中更新 `consumes.version` 与 snapshot；consumer CI 跑 `hek workspace compat --contract <id>`，比对 provider 产物与本地 snapshot。
5. 每个 unit 的 `governance.workspace` 都引用完整的 related change set；`derived_from` 指向直接上游 provider change_id。
6. 各 unit 独立 Apply、Verify、Sync、Archive；任一 unit 的 spec 或 change 未通过，不得宣称 federation change set 完成。
```
...
加载分层与硬预算：现有 index/detail 上限保持不变；联邦模型新增 `MAX_INPUT_BYTES` 门禁：

| 层 | 内容 | 进入条件 | 预算 |
| --- | --- | --- | --- |
| L0 常驻 | 根 adapter、`agent-policy.yaml`、`profile.yaml`、`.hek/context/ai.json` | 每次任务 | index ≤ 8192 字节（`MAX_INDEX_BYTES`），modules ≤ 50（`MAX_MODULES`）；root adapter 会话常驻但不计入 `input_bytes` |
| L1 路径触发 | owning module 的 `AI.md` 与祖先链 | 目标路径前缀匹配 | 每份 ≤ 800 行 / 65536 字节 |
| L2 关键词触发 | `read_when` 命中的其他 module | 关键词精确命中 | 命中才追加 |
| L3 按需检索 | 契约 snapshot、历史 change、经验条目 | 跨工程或踩坑时 | 检索进入，收敛到具体文件；单任务总 `input_bytes` 不得超过 `MAX_INPUT_BYTES=262144` |

规则：

- workspace 层不持有任何上下文文档，不建第二份 `AI.md`。
- 跨工程任务的常驻上下文 = coordinator 的 L0/L1 + 契约 snapshot，不得挂载其他 unit 的 L1。
- 复用 `scripts/context_cache.py` 的 `input_bytes` 与 hit/miss/bypass 度量；`input_bytes` 只统计 `resolve_context.py` 返回的可信上下文文件，root adapter 是会话常驻内容，不重复计入；长任务命中率沿用默认门槛 `0.995`。
...
| 聚合预演（可选） | 任一 unit 仓的手动触发任务 | checkout 全量 unit 后 `hek workspace verify`（I3、I4、I6、I10、I11、I14、I15） |
...
- `openspec/` 仍由各 unit 仓各自持有；跨工程变更由每个参与 unit 各自开 change 和 spec，通过 `governance.workspace.related_changes` 关联，`derived_from` 仅表示直接上游关系。
- A/B/C 三工程关联时，必须能在三仓分别定位到三份本地 spec；任何汇总 spec 只能作为人工阅读材料，不参与 OpenSpec 门禁，也不是权威事实。
...
- `.git` 既可能是目录，也可能是 worktree/submodule 指针文件；候选枚举不得只接受目录形式。
...
| 层 | 机制 | 失败表现 |
| --- | --- | --- |
| 会话入口 | 任何写操作前运行 `hek workspace guard --path <target> --session-root <session> --json` | 退出码 2，Agent 停止并报告 |
...
| `hek workspace verify [--root <dir>]... [--json]` | 校验 I1–I7、I10、I12、I14、I15 | P0/P1 |
| `hek workspace context <path> [--json]` | 回答路径归属、应加载上下文、是否跨工程、预估字节数 | P0 |
| `hek workspace exec <unit> -- <cmd...>` | 切到 unit 根启动 CLI，注入 `HEK_UNIT` / `HEK_WORKSPACE` / `HEK_SESSION_ROOT` | P0 |
| `hek workspace guard --path <path> [--session-root <dir>] [--json]` | 写入前门禁：判定是否嵌套或越界，blocked 即禁止编码（I14、§5.10）；未提供 `--session-root` 时使用 `HEK_SESSION_ROOT`，再回退到 cwd | P0 |
...
- `VALUE_OPTIONS`（第 50 行）增加 `--root`、`--depth`、`--contract`、`--unit`、`--path`、`--session-root`、`--unit-id`。
- `parseArgs()`（第 119 行起）增加 workspace 子命令和位置参数解析：`workspace` 后第一个 token 是 `discover/verify/context/exec/guard/compat/graph/run/pin`，后续位置参数写入 `positionals`；`--` 后所有 token 写入 `result.options.rest`。`--root` 必须累积为数组，不能覆盖前一个值。
- `main()`（第 872 行起）在既有 if 链末增加 `if (parsed.command === 'workspace') return runWorkspace(parsed.options);`。
- `usage()` 增加 workspace 段落；`exec` 的 session root 传入并写入 `HEK_SESSION_ROOT`。
- 新增 `runWorkspace()`，通过 Python 子进程调用 `scripts/workspace_ctl.py`，保持 Python 实现为唯一逻辑源；不要复用只接受 onboard mode 的现有 `invoke()`。
- `baseArgs()` 透传 `unitId` 到 `onboard.py`，使 `hek init --unit-id <id>` 与直接调用 Python 入口保持一致。
...
| `scripts/workspace_guard.py` | `git_toplevel(path: Path) -> Path`；`locate_harness_root(path: Path) -> Path or None`；`detect_nesting(roots: list[Path]) -> list[Diagnostic]`；`guard(target: Path, session_root: Path) -> list[Diagnostic]`；session root 必须来自 CLI、`HEK_SESSION_ROOT` 或当前 cwd |
...
| `templates/fitness/check_context_budget.py.template` | 校验单任务 `input_bytes <= MAX_INPUT_BYTES` 与跨 unit 上下文引用（I11） |
...
| `tests/test_workspace.py`、`tests/test_workspace_ctl.py`、`tests/test_workspace_cli.js` | 见第 8 节；Node 测试覆盖 wrapper 的参数和位置参数透传 |
| `tests/fixtures/workspace/**` | 夹具：`single/`、`pair-ok/`、`triple-ok/`、`pair-orphan/`、`pair-uncloned/`、`nested/`、`nested-no-identity/`、`nested-cross/`、`foreign-methodology/`、`version-mismatch/`、`untracked-identity/`、`cycle/` |
...
| `scripts/onboard.py` | `--check` 末尾追加 identity 校验（单 Git 仓无 identity 时跳过，联邦模式缺失时 blocked）；新增 `--unit-id` 用于生成 identity 草稿；`project_root()`（第 179 行）三态判定（§5.3 第 4 条） | 中，必须保证单仓无 identity 时行为字节级一致 |
...
| `scripts/check_change_workspace.py` | 校验 `governance.workspace`：字段集合、`role` 枚举、`related_changes` 的 unit/change/spec 一一对应、禁止 workspace 根 spec、`breaking` 与 `derived_from` 的联动、`verification` 文件存在性 | 中，旧 change 无该键时跳过 |
...
| `tests/test_cli.js`、`package.json` | 将 workspace wrapper 测试纳入 `test:node`，并覆盖位置参数、重复 `--root`、`--` 透传 | 中 |
| `docs/versioning.md`、`migrations/releases.json`、`VERSION`、`package.json` | 1.0.0 版本与迁移条目；Kit `VERSION` 与 npm metadata 必须一致，迁移清单中的版本来源同步到当前布局 | 中 |
...
| `triple-ok/` | 三个平级仓，各自有一个 change 和至少一份 spec，related_changes 互相对齐 | 三工程三 spec |
| `pair-orphan/` | provider 仓可见但没有 publishes 对应 contract 的 consumer | I5 blocked |
| `pair-uncloned/` | provider_repo 不在可见仓集合，consumer 仅产生 warning | I5 warning |
| `nested/` | unit B 的仓根位于 unit A 仓根内 | I4 |
| `nested-no-identity/` | 嵌套仓缺少 identity，仍必须被结构门禁发现 | I4、I14 |
...
| `test_three_units_have_three_local_specs` | `triple-ok/` 的三个 unit 各有一份本地 spec，projection 不生成 workspace spec |
...
| `test_guard_blocks_nested_write` | 显式传入会话根后，嵌套路径上 `guard` 返回 `nested.detected` 且退出码 2 |
| `test_guard_blocks_cross_repo_write` | 显式传入 A 的 `session_root`，guard B 仓路径返回 `boundary.cross-repo` |
...
| `test_verify_orphan_contract` | `pair-orphan/` 报 `contract.orphan` 且退出码 2 |
| `test_verify_uncloned_provider_warning` | `pair-uncloned/` 报 warning 且不因未 checkout provider 阻断 |
...
| `test_cross_unit_path_rejects_bare` | 跨 unit 结构中的裸相对路径被 I7 拒绝；identity 内本 unit 路径允许 |
| `test_exec_sets_env_and_cwd` | `exec` 注入 `HEK_UNIT`/`HEK_WORKSPACE` 且 cwd 为 unit 根 |
| `test_cli_workspace_positionals_and_separator` | Node wrapper 正确解析 workspace 子命令、位置参数、重复 `--root` 和 `exec -- <cmd...>` |
| `test_three_state_root_detection` | workspace 根调用未指定 unit 时报错退出，不判定 fresh |
| `test_context_budget_exceeded` | 超预算返回 blocked |
| `test_digest_changes_on_kit_version_or_tracking` | 仅修改 `.hek/VERSION` 或 identity tracking 状态时 digest 变化 |
| `test_mixed_workspace_roots_blocked` | roots 包含多个 `workspace_id` 时返回 `workspace.mixed` 且退出码 2 |
| `test_spec_reference_must_match_local_change` | related change 的 change_id、unit 或 spec 路径不一致时返回 `spec.reference` 并退出码 2 |
| `test_missing_external_spec_blocked` | 聚合 verify 找不到 related change 的成员 spec 时返回 `spec.missing` 并退出码 2 |
...
1. `triple-ok/` 上跑 `hek workspace verify --json` → 三个 unit 各有一个本地 change 和至少一份 spec，related_changes 完整，projection 不含 workspace spec。
2. `pair-ok/` 上跑 `hek workspace discover --json` → projection 含两个 unit、一条契约边。
3. 修改 provider 契约版本，consumer 未更新 → consumer 的 `compat` 退出码 2。
4. consumer 更新 `consumes.version` 与 snapshot 后 → `compat` 退出码 0，产出证据文件。
5. 在 consumer 仓跑 `hek check` → 既有校验与新 identity/spec 校验同时通过。
...
| 多个平级独立仓 | 联邦 | 各仓新增 `.hek/project/identity.yaml`（`hek init --unit-id <id>` 生成草稿），补 `.gitignore` 片段 |
...
交付：`workspace.py`、`workspace_guard.py`、`check_identity.py`、`workspace_ctl.py` 的 `discover/verify/context/exec/guard`、`identity.yaml.template`、`templates/hooks/deny-nested-write.json.template`、`.gitignore` 片段、`core/workspace-federation.md`、`onboard.py` 安装判定的证据化收紧（§5.10 反面教材）、`bin/harness-engineering-kit.js` 的 workspace 参数解析与 wrapper、夹具、Python/Node 测试与 8.2 中 P0 相关用例。
...
python scripts/workspace_ctl.py guard --path tests/fixtures/workspace/nested/backend-api/backend-ui/src/x.ts --session-root tests/fixtures/workspace/nested/backend-api --json   # 退出码 2，code=nested.detected
python scripts/workspace_ctl.py guard --path tests/fixtures/workspace/pair-ok/backend-api/src/x.java --session-root tests/fixtures/workspace/pair-ok/backend-api --json           # 退出码 0
node bin/harness-engineering-kit.js workspace discover --root tests/fixtures/workspace/pair-ok --json # 与 Python 入口结果一致
...
交付：`compat` 与 `graph`、`check_contract_pin`、`workspace.compat` 证据、consumer CI 模板、`governance.workspace` 及 `related_changes`/本地 spec 引用校验、`resolve_context` 的 `unit_id`/`cross_unit` 输出、`check_context_budget`。

验收：8.3 端到端五步全部通过；`triple-ok` 验证三工程三份本地 spec；`pair-orphan`、`cycle`、`version-mismatch` 三个夹具在 `verify` 下退出码 2。
