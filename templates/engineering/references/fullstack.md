...
Read both backend and frontend references plus `.hek/kit/core/fullstack-profile.md`.

The coordinator owns one shared behavior contract covering versioning, nullability, validation, errors, authorization, idempotency, retry/timeouts, timezone/precision, and observability. Follow `.hek/kit/core/design-review.md`; produce backend Model and frontend Decompose sections in one Design artifact. The topology must show both sides, shared/generated contracts, external dependencies, and ownership. Runtime flows must show the end-to-end success path and material error, permission, retry, and timeout paths.
