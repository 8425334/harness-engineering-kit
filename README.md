...
Interactive `init` opens the selected Agent first and lets that Agent perform onboarding; enter `0` or `skip` at the prompt for the deterministic flow, and it falls back automatically when no agent is installed. Non-interactive runs never launch an external process unless `--open` is supplied, and `--open` there requires `--agent`/`HEK_AGENT`. `--json` switches to machine-readable output: without `--yes` init prints the read-only plan and exits 2; with `--yes` it applies, checks, and prints one JSON receipt. Use `HEK_AGENT` instead of `--agent`, or `--prompt` to customize the first prompt sent to terminal agents.
...
`hek init` is Agent-driven: it selects an installed Agent first, opens that Agent's CLI in the resolved project root, and passes the Kit path plus the onboarding contract. The Agent reads project facts, generates the read-only plan, asks for confirmation, fills project-specific values, applies the canonical script, and runs deterministic checks. Tier 1 installs the core control plane, including the production policy scaffold referenced by `agent-policy.yaml`; the default Tier 2 additionally installs Fitness gate scripts, Fitness rules, and lesson memory. Each run writes `docs/methodology/onboarding.json` with the source version, file digests, created/updated/preserved files, and verification result. Use `--direct` only when a headless deterministic install is explicitly wanted; it ignores `--agent` and `HEK_AGENT`.
