...
7. Run the post-apply check and stop on failure:
   ...
   `python3 .hek/kit/scripts/context_cache.py report --root . --json`
   to verify the measured provider rate. `bypass` is reported separately and
   is not included in the hit-rate denominator.

8. Report the onboarding state, changed files, preserved legacy files, check output, unresolved placeholders or manual decisions, and the next valid action. Save the machine-readable receipt at `.hek/state/onboarding.json`.
9. When the repository already has an `openspec/config.yaml`, merge the OpenSpec-owner/Engineering-governance rules (see the installed `openspec/README.md`) into it, preserving the project's existing `context:` and `rules:`. Keep Harness guidance outside files generated by OpenSpec integration updates.
10. If the repository contains live OpenSpec change directories that do not use the `harness-engineering` schema or lack `governance.json`, report them. Recreate or explicitly migrate the OpenSpec change before attaching governance with `init_governance.py`; an already approved change requires a new approval when its schema or contract artifacts change.
...
Every apply records the content identity of the Kit it installed in `.hek/state/kit-identity.json` (version plus a `source_fingerprint` over the shipped assets). Equal versions are not proof of equal content: when the Kit content changes without a version bump, or an installed canonical resource is edited or lost, the plan reports `identity_relation: drift` with a `version.same-content-drift` warning and `--check` fails naming the differing files. The remedy is always the same: re-run the plan and apply from the checkout the project is meant to run, and bump `VERSION` when the content change is a release.
...
- `--uninstall` reverses onboarding and is also read-only until `--apply`. It deletes only assets recorded in `.hek/state/onboarding.json` whose digests still match, keeps files the install preserved or that were edited afterwards, prunes emptied directories, and writes `.hek/state/uninstall.json`. Prefer `hek uninstall [--keep-project-facts]` so the CLI supplies the confirmation flow, and treat any removal outside the receipt as a separate, explicitly approved change.
...
- No command performs `git reset`, deletes legacy entries outside an explicit uninstall, changes `.hek/fitness/**` after bootstrap, installs dependencies, or accesses production systems.
