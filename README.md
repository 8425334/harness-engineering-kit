...
`guard --stdin` is the same gate for a host agent's write hook: it reads the tool payload (for example `{"tool_input":{"file_path":"src/app.ts"}}`) from stdin and denies the call with exit 2 when the target is nested or belongs to another unit. A payload it cannot interpret is reported as information rather than treated as a violation, so an unfamiliar hook runtime never blocks every write.
...
Version-aware upgrades compare the installed `.hek/VERSION` with the Kit version, synchronize all canonical resources for lower-to-higher upgrades, block downgrades, and report any release-specific migration review. Equal versions are not taken as proof of equal content: every checkout has a `source_fingerprint` over the shipped assets, every apply records it in `.hek/state/kit-identity.json`, and the plan, `hek check`, and `hek doctor` report `identity_relation` with the exact drifting files instead of silently accepting a same-version update. See [Versioning and Upgrades](docs/versioning.md).
