...
`discover` and `verify` report units, contract edges and diagnostics as JSON and exit 2 when blocked. `guard` decides whether a write target belongs to the current session from Git facts alone, so a nested unit or a sibling repository is blocked before any code is written. Engineering-level nesting is no longer supported; the split migration may use a one-off, time-boxed waiver, but `verify` keeps reporting blocked until the structure is actually split. Each participating unit owns its own OpenSpec change and spec — when A, B and C participate there are three local specs linked through `governance.json.workspace.related_changes`, and the workspace root never owns a spec. See [Workspace Federation](core/workspace-federation.md).
